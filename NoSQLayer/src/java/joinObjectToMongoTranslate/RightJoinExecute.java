...
public class RightJoinExecute {

    public String executeRightJoin(SelectClause selectStatement, String header) {
        ...
        JSONArray array_right = null, array_left;
        String join_result = "";
        //Tabela do right Join
        String table_right_join = selectStatement.getTablesQueried().get(1).getName();
        boolean first_is_right = false;      //Identifica se a primeira tabela é tambem a left
        ...
        if (!selectStatement.getOrdenacao().isEmpty()) {
            ...
            if (selectStatement.getOrdenacao().size() == 1) {
                ...
                if (selectStatement.getOrdenacao().get(0).getTableReferenciada().equals(table_right_join)) {
                    first_is_right = true;
                } else {
            ...
            if (table_right_join.equals(selectStatement.getJoinList().get(0).getTableRightExpression())) {
        ...
        /*
         *   Otimização Possivel: Tentar ja retornar o result set final (tentar utilizar MAP)
         */
    ...
    public String executeRightJoinSubDoc(SelectClause selectStatement, String header, String subdoc) {
        DBCollection collection = SelectObjectWithJoinTranslate.database.getCollection(selectStatement.getJoinList().get(0).getTableLeftExpression());
