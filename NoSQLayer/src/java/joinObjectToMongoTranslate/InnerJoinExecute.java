...
public class InnerJoinExecute {

    public String executeInnerJoin(SelectClause selectStatement, String header) {
        ...
        MyRunnable my1 = new MyRunnable(lista_resultados2, array_header, lista_resultados.subList(0, num_tarefas_por_thread), compartilha, first_param.getName(), second_param.getName(),"INNER");
        MyRunnable my2 = new MyRunnable(lista_resultados2, array_header, lista_resultados.subList(num_tarefas_por_thread + 1, 2 * num_tarefas_por_thread), compartilha, first_param.getName(), second_param.getName(),"INNER");
        MyRunnable my3 = new MyRunnable(lista_resultados2, array_header, lista_resultados.subList(2 * num_tarefas_por_thread + 1, 3 * num_tarefas_por_thread), compartilha, first_param.getName(), second_param.getName(),"INNER");
        MyRunnable my4 = new MyRunnable(lista_resultados2, array_header, lista_resultados.subList(3 * num_tarefas_por_thread + 1, lista_resultados.size() - 1), compartilha, first_param.getName(), second_param.getName(),"INNER");
    ...
    public String executeInnerJoinSubDoc(SelectClause selectStatement, String header, String subdoc) {
        DBCollection collection = QueryInterceptor.database.getCollection(selectStatement.getJoinList().get(0).getTableRightExpression());
        DBObject criteria = new BasicDBObject(subdoc, new BasicDBObject("$exists", true));
        DBCursor resultado = collection.find(criteria);
        String join_result = "";
